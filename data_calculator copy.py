...
class DataCalculator(QObject):
    ...
    def calculate_all(self, ticker):
        ...
        indicators_file_path = f"indicators_{ticker}.csv"
    ...
    def calculate_psar2(self, df):
        ...
        try:
            ...
            # Calculate PSAR with acceleration 0.01 and maximum 0.1

            psar_result2 = ta.psar(df['High'], df['Low'], df['Close'], acceleration=0.01, maximum=0.1)
            
            # Check if psar_result is not empty and has columns
            if psar_result2.empty or psar_result2.columns.size == 0:
                self.logger.log_or_print("PSAR calculation failed. The result is empty or has no columns.", level="ERROR")
                return
            
            # Log the columns being added for clarity
            self.logger.log_or_print(f"Adding the following PSAR columns to the DataFrame: {psar_result2.columns.tolist()}", level="INFO")
            #rename the columns to reflect the new parameters from  ['PSARl_0.02_0.2', 'PSARs_0.02_0.2', 'PSARaf_0.02_0.2', 'PSARr_0.02_0.2'] to ['PSARl_0.01_0.1', 'PSARs_0.01_0.1', 'PSARaf_0.01_0.1', 'PSARr_0.01_0.1']
            psar_result2.columns = ['PSARl_0.01_0.1', 'PSARs_0.01_0.1', 'PSARaf_0.01_0.1', 'PSARr_0.01_0.1']
            # Add each PSAR column to the original DataFrame with its default name
            for column in psar_result2.columns:
                df[column] = psar_result2[column]

            self.logger.log_or_print("PSAR calculation and column addition completed successfully.", level="INFO")

        except Exception as e:
            self.logger.log_or_print(f"An error occurred in calculate_psar2: {str(e)}", level="ERROR", exc_info=True)
    """
    def calculate_psar(self, df, acceleration=0.02, maximum=0.2):
        """
        Calculate the Parabolic Stop and Reverse (PSAR) manually and add it to the DataFrame.
        ...
        try:
            ...
            # Initialize the first row of PSAR
            df['PSARl_0.02_0.2'] = df['Low']
            df['PSARs_0.02_0.2'] = df['High']
            df['PSARaf_0.02_0.2'] = acceleration
            df['PSARr_0.02_0.2'] = True # True for rising, False for falling
            ...
            for i in range(1, len(df)):
                reverse = False
                high, low = df['High'][i], df['Low'][i]
                
                if df['PSARr_0.02_0.2'][i-1]:  # If the trend was rising
                ...
                if not reverse:
                    if df['PSARr_0.02_0.2'][i-1]:
                        if high > ep:
                            ep = high
                            af = min(af + acceleration, maximum)
                        psar = min(psar, low, df['Low'][i-1])
                    else:
                        if low < ep:
                            ep = low
                            af = min(af + acceleration, maximum)
                        psar = max(psar, high, df['High'][i-1])

                df.loc[i, 'PSARl_0.02_0.2'] = psar if df['PSARr_0.02_0.2'][i-1] else None
                df.loc[i, 'PSARs_0.02_0.2'] = None if df['PSARr_0.02_0.2'][i-1] else psar
                df.loc[i, 'PSARaf_0.02_0.2'] = af
                df.loc[i, 'PSARr_0.02_0.2'] = not reverse
    ...
    def calculate_atr(self, df):
        ...
        #calculate Rolling Standard Deviation for 10 days and 50 days based on close price
