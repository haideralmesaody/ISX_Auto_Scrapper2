...
class FileManagerASE:
    def __init__(self):
        self.logger = Logger()
        script_dir = os.path.dirname(os.path.abspath(__file__))
        csv_path = os.path.join(script_dir, "TICKERS_ASE.csv")
        try:
    ...
    def generate_report(self, ticker):
        ...
        """
        #get df from ticker_indicators.csv
        self.logger.log_or_print(
            f"generate_report: Generating report for {ticker}.",
            level="INFO",
            module="FileManager",
        )
        #CHECK IF THEFILE EXIST
        if not os.path.isfile(f"ASE_indicators_{ticker}.csv"):
            self.logger.log_or_print(
                f"generate_report: File ASE_indicators_{ticker}.csv does not exist.",
                level="ERROR",
                module="FileManager",
            )
            return None
        else:
            df=pd.read_csv(f"ASE_indicators_{ticker}.csv")
            try:
                ...
                if df is not None:
                    ...
                    try:
                        ...
                        embedded_filename = f"{now.strftime('%Y-%m-%d %H-%M-%S')}_ASE_{ticker}_report_embedded.html"
    ...
    def get_company_name(self, ticker):
        ...
        """
        try:
            return self.df_tickers[self.df_tickers["Ticker"] == ticker]["Company's name"].values[
                0
            ]
        except IndexError:
