...
class DataCalculator(QObject):
    ...
    def calculate_psar2(self, df, acceleration=0.01, maximum=0.1):
        """
        Calculate the Parabolic Stop and Reverse (PSAR) manually and add it to the DataFrame.
        ...
        try:
            ...
            # Initialize the first row of PSAR
            df['PSARl_0.01_0.1'] = df['Low']
            df['PSARs_0.01_0.1'] = df['High']
            df['PSARaf_0.01_0.1'] = acceleration
            df['PSARr_0.01_0.1'] = True # True for rising, False for falling
            ...
            for i in range(1, len(df)):
                ...
                if df['PSARr_0.01_0.1'][i-1]:  # If the trend was rising
                    psar = psar + af * (ep - psar)
                    if low < psar:
                        psar = ep
                        reverse = True
                ...
                df.loc[i, 'PSARl_0.01_0.1'] = psar if df['PSARr_0.01_0.1'][i-1] else None
                df.loc[i, 'PSARs_0.01_0.1'] = None if df['PSARr_0.01_0.1'][i-1] else psar
                df.loc[i, 'PSARaf_0.01_0.1'] = af
                df.loc[i, 'PSARr_0.01_0.1'] = not reverse
                
            self.logger.log_or_print("PSAR2 calculation and column addition completed successfully.", level="INFO")
